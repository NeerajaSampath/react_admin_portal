...
const Decline = ({ ViewConfig }) => {
  const [DeclinedState, setDeclinedState] = useState([]);
  ...
  const DeclinedAPI = async () => {
    try {
      setLoading(true);
      const res = await axios({
        method: "get",
        url: `${GetStatusConfig}/declined`,
      });
      setLoading(false);
      let formate_data = LowercaseFn(res.data.data.configStatusRequestList);
      setDeclinedState(formate_data);
  ...
  const columns = [
    { field: "uuidList", headerName: "Config Id", flex: 1 },
    { field: "configName", headerName: "Config Name", flex: 1 },
    { field: "moduleName", headerName: "Module Name", flex: 1 },
    {
      ...
      renderCell: (params) => (
        <>
          <span className="decline_text">{params?.value}</span>
        </>
      ),
    },
    {
